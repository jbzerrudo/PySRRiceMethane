...
# ─── Configuration ────────────────────────────────────────────────────────────
# =====================================================================
# CONSTRAINED JPN VARIANT of PySR7v2 (prepped to find a stable nonlinear AUC form).
# Only the input paths + two constraint dicts differ from PySR7v2; the GAM-RF/VIF
# predictors, 12-seed loop, day-grouped split, and Stage-8 generation are unchanged,
# so outputs are directly comparable to the original JPN run.
# Constraints added to force IDENTIFIABLE forms (JPN's instability came from nested
# rationals + saturated tanh-in-exp): no division-in-division (denominator <= 8 nodes);
# no tanh-in-exp / exp-in-tanh; tanh and exp stay shallow. If the winners still reduce
# to ~linear (CV ~0.43, stable), that CONFIRMS linear is the stable optimum for JPN.
# =====================================================================
INPUT_DIR = r"C:\Users\zerru001\OneDrive - Wageningen University & Research\Paper1\CSV\JPN"
OUTPUT_DIR = r"C:\Users\zerru001\OneDrive - Wageningen University & Research\Paper1\RUN2\PYSR\JPN\pysr7v2_constrained"
INPUT_FILE = "JPN-MSE_2012_retainedvars2.csv"
TARGET_COL = "F_CH4_F"
...
NESTED_CONSTRAINTS = {
    "exp":  {"exp": 1, "log": 0, "tanh": 0},   # CONSTRAINED (JPN): also no tanh inside exp
    "log":  {"log": 0, "exp": 1},
    "tanh": {"tanh": 0, "exp": 0},             # CONSTRAINED (JPN): tanh stays shallow
    "/":    {"/": 0},                          # CONSTRAINED (JPN): NO division inside division
}
...
CONSTRAINTS = {
    ...
    "-":    (-1, -1),
    "*":    (-1, -1),
    "/":    (-1, 8),   # CONSTRAINED (JPN): denominator <= 8 nodes
}
