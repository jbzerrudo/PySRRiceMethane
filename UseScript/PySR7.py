"""
Symbolic Regression for Rice Methane Equation Discovery (v6.4 — "PySR7")
Changes from v6.3:
  - Stage 8 CV scoring: added POOLED out-of-fold R²/RMSE/MAE as the headline
    metric. One score per equation over every test record from all 5 day-
    folds combined (single global denominator), reusing compute_metrics on
    the existing per-record frame (rec_df). This removes the distortion from
    low-dynamic-range folds that inflated the old mean-of-per-fold R²
    (e.g. the "0.47 ± 0.36" artifact). The per-fold mean ± SD is RETAINED,
    relabeled as a diagnostic. Equations, coefficients, folds, day-grouping,
    seeds, and Rule A are all UNCHANGED. New output: cv_pooled_metrics.csv.
  - Removed the "sqrt": {"sqrt": 0} nesting constraint, so sqrt(sqrt(x))
    (= x**0.25) is now reachable (no general power operator otherwise).
Changes from v6.2:
...
# ─── Configuration ────────────────────────────────────────────────────────────
INPUT_DIR = r"C:\Users\zerru001\OneDrive - Wageningen University & Research\Paper1\CSV\KOR\postgamrf2_run"
OUTPUT_DIR = r"C:\Users\zerru001\OneDrive - Wageningen University & Research\Paper1\PYSR\KOR"
INPUT_FILE = "KOR-CRK_2018.0_retdvars2.csv"
TARGET_COL = "F_CH4_F"
EXCLUDE_HEADERS = {"Date", "Deltime", "time", "F_CH4_F_orig"}
...
def _write_auto_stage8(forms_for_stage8, predictors, input_csv,
    ...
    # v6.4: pooled out-of-fold R² blocks (HEADLINE metric). Reuses
    # compute_metrics over the concatenated per-record frame (rec_df) so a
    # single low-variance fold cannot distort the score. Unrolled per slot,
    # mirroring summary_block_r2; nested under `if all_residuals:` (8-space).
    summary_blocks_pooled = []
    for s in summary_lines:
        summary_blocks_pooled.append(
            f'        _yt = rec_df[TARGET_COL].to_numpy(dtype=float)\n'
            f'        _yp = rec_df["{s}_pred_refit"].to_numpy(dtype=float)\n'
            f'        _r2, _rmse, _mae = compute_metrics(_yt, _yp)\n'
            f'        _n = int(np.isfinite(_yp).sum())\n'
            f'        lines.append(f"  {s:<{slot_w}} pooled R² = {{_r2:+.3f}}   '
            f'RMSE = {{_rmse:.3f}}   MAE = {{_mae:.3f}}   (n={{_n}})")\n'
            f'        _pooled_rows.append({{"slot": "{s}", "pooled_R2": _r2, '
            f'"pooled_RMSE": _rmse, "pooled_MAE": _mae, "n_records": _n}})'
        )
    summary_block_pooled = "\n".join(summary_blocks_pooled)
...
{summary_block}
    ...
    # Pooled out-of-fold R² — HEADLINE metric (v6.4). One R² per equation over
    # every test record from all folds combined, so a single low-variance fold
    # can no longer distort the score. Equations/coefficients are unchanged.
    lines.append("")
    lines.append("Pooled out-of-fold R²  (all folds combined — HEADLINE, "
                 "robust to low-variance folds):")
    _pooled_rows = []
    if all_residuals:
{summary_block_pooled}
        if _pooled_rows:
            pd.DataFrame(_pooled_rows).to_csv(
                os.path.join(OUTPUT_DIR, "cv_pooled_metrics.csv"), index=False)
            print(f"  [SAVED] cv_pooled_metrics.csv ({{len(_pooled_rows)}} rows)")
    else:
        lines.append("  (per-record residuals unavailable; pooled metric skipped)")

    lines.append("")
    lines.append("Per-fold R²  (mean ± SD across the 5 folds — DIAGNOSTIC; "
                 "can be unstable on low-variance folds):")
{summary_block_r2}
