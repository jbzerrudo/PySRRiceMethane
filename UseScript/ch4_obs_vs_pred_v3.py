...
PH-IR and JP-MSE use their FULL-RECORD forms and files (unchanged). The SK-CRK
panel uses the GROWING-SEASON subset run: Eq. 5 (PySR auto-best, AUC_wet form)
evaluated on KOR-CRK_2018_updated_retvars2.csv (9 Apr-30 Sep 2018). This puts
all three panels on a common active-season footing.
...
OUTPNG = r"C:\Users\zerru001\OneDrive - Wageningen University & Research\Paper1\RUN2\Graphs\000_RICEMETHANE\ch4_obs_vs_pred_timeseries_v2.png"
...
def predict_kor_gs(df):
    # eq:kor-gs (GROWING SEASON)   F = d*(AUC_wet + b/(a + g/sqrt(AUC_wet + c)))*(SR + e)
    # PySR auto-best (#15, seed 42) -- the form reported as Eq. 5.
...
SITES = {
    ...
    "SK-CRK": dict(fn=predict_kor_gs, color="#762a83", dayfirst=False),   # growing-season subset (Eq. 5)
...
for ax, (site, cfg) in zip(axes, SITES.items()):
    ...
    title = site if site != "SK-CRK" else "SK-CRK (growing season)"
