...
INPUT_CSV = r"C:\Users\zerru001\OneDrive - Wageningen University & Research\Paper1\RUN2\CSV\KOR\21Jul26\KORCRK_GS1_2018_RETAINED_VARS_postgam1.csv"
OUTPUT_DIR = r"C:\Users\zerru001\OneDrive - Wageningen University & Research\Paper1\RUN2\COLLINCHECK\KOR\21JUL26"
SITE_NAME = "KOR-CRK_2018_GS"  # used in output filenames; e.g. "SK-CRK-2016"

TARGET_COL = "F_CH4_F"
VIF_THRESHOLD = 5.0 # was 10.0
EXCLUDE_HEADERS = ["Date", "Deltime", "time", "F_CH4_F_orig"]
MISSING_FLAGS = [-9999, -999900, -99999]
DPI = 300
ADD_TIMESTAMP = True

# Importance-guided VIF elimination.
# Point IMPORTANCE_DIR at the NEW GAM-RF output folder FOR THIS SEASON; the latest
# predictor_perm_importance_*.csv inside it is picked automatically (no timestamp to
# type). Switch it together with INPUT_CSV / SITE_NAME when you change season:
#   full season    -> ...\RUN2\GAMRF\KOR\0_review_21Jul26\hampel\full
#   growing season -> ...\RUN2\GAMRF\KOR\0_review_21Jul26\hampel\growingseason
import glob, os
IMPORTANCE_DIR = r"C:\Users\zerru001\OneDrive - Wageningen University & Research\Paper1\RUN2\GAMRF\KOR\0_review_21Jul26\hampel\growingseason"
_imp_hits = sorted(glob.glob(os.path.join(IMPORTANCE_DIR, "predictor_perm_importance_*.csv")))
IMPORTANCE_CSV = _imp_hits[-1] if _imp_hits else None  # newest file; None -> blind VIF fallback
print(f"[IMPORTANCE] {IMPORTANCE_CSV if IMPORTANCE_CSV else 'NONE FOUND in ' + IMPORTANCE_DIR + ' -> blind VIF'}")
...
def analyze_collinearity(df, target_col, threshold=5.0, outdir=None,
