...
OUTPUT_DIR = r"C:\Users\zerru001\OneDrive - Wageningen University & Research\Paper1\RUN2\PYSR\JPN\pysr7v3_jpn"
...
TEST_FRACTION = 0.2
# Internal PySR hold-out split mode (does NOT change data resolution; rows stay
# half-hourly either way — this only controls which records go to train vs test).
#   True  = day-grouped 80/20 (whole days held out; removes within-day
#           autocorrelation leakage; honest but lower R²).
#   False = record-level 80/20 (random by half-hourly record, like v5;
#           optimistic R² due to within-day near-twins in train).
# NOTE: this flag only affects the candidate-ranking hold-out inside
# run_symbolic_regression. The Stage 8 generated CV script stays day-grouped.
SPLIT_BY_DAY = False
PARSIMONY = 0.0032
BATCHING = True
BATCH_SIZE = 256
...
def run_symbolic_regression(csv_path, out_root, run_ts, seed=None, outdir=None):
    ...
    # Gated by SPLIT_BY_DAY (config): False -> record-level (half-hourly) split.
    if SPLIT_BY_DAY and dates_full is not None:
