...
INPUT_CSV = r"C:\Users\zerru001\OneDrive - Wageningen University & Research\Paper1\CSV\KOR\KOR-CRK_2018_cm.0.csv"  # ← adjust path to your CSV file
SITE_NAME = "KOR-CRK_2018"  # Used in plot titles and output filenames
OUTPUT_DIR = r"C:\Users\zerru001\OneDrive - Wageningen University & Research\Paper1\RUN2\Graphs\KOR\2018_new_cm"  # ← adjust path to your desired output directory

# Depth unit: "m" for most sites, "cm" for PH-IR
DEPTH_UNIT = "cm"    # ← set to "cm" for PH-IR
...
def make_focused_plot(df, site_name):
    ...
    for i, var in enumerate(available):
        ...
        series = df[var]
        if var == "depth":
            # paddy water depth is always sub-metre, so |values| > 1 means the
            # column is stored in cm; convert it to whatever DEPTH_UNIT asks for
            stored = "cm" if series.abs().quantile(0.99) > 1.0 else "m"
            if stored == "cm" and DEPTH_UNIT == "m":
                series = series / 100.0
            elif stored == "m" and DEPTH_UNIT == "cm":
                series = series * 100.0
