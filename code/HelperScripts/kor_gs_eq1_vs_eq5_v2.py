...
def main():
    ...
    if have_eq1 and have_eq5:
        e1m, e1sd, e1mx, e1f, _ = results["eq1_refit"]
        e5m, e5sd, e5mx, e5f = results["eq5_refit"]
        both = np.isfinite(e1f) & np.isfinite(e5f)
        wins, nval = int(np.sum(both & (e1f > e5f))), int(np.sum(both))
        print("=" * 70)
        ...
        if np.isfinite(gap):
            ...
            print(f"\n  Eq. 1 {verdict} Eq. 5 in-season by {gap:+.3f} CV R2; "
                  f"Eq. 1 > Eq. 5 in {wins}/{nval} folds.")
            if gap > 0:
        ...
        # ---- seasonal-index diagnostic: are AUC_wet and temperature interchangeable? ----
        print("\n" + "=" * 70)
        print("SEASONAL-INDEX DIAGNOSTIC  (does temperature simply re-express AUC_wet?)")
        print("=" * 70)
        sub = df[["_AUCwet", COLS["asinh_Ta"]]].dropna()
        pear = float(sub["_AUCwet"].corr(sub[COLS["asinh_Ta"]]))
        spear = float(sub["_AUCwet"].corr(sub[COLS["asinh_Ta"]], method="spearman"))
        print(f"  corr(AUC_wet, asinh_Ta):  Pearson {pear:+.3f}   Spearman {spear:+.3f}")
        for tcol in ("Tair", "Tsoil"):
            if tcol in df.columns:
                t = df[["_AUCwet", tcol]].dropna()
                print(f"  corr(AUC_wet, {tcol}):{' '*(6-len(tcol))}    Pearson "
                      f"{float(t['_AUCwet'].corr(t[tcol])):+.3f}")
        print("  High |corr| (say > 0.7) => AUC_wet and temperature are near-interchangeable")
        print("  seasonal indices in-season (the tidiest reading: the choice of AUC_wet is")
        print("  representational). Low => temperature fits via a distinct signal.")
    print()
