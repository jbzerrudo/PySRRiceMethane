...
Axes now span the full data range on both x and y (no percentile clipping), so
the high-flux observed excursions and the negative tail are shown in full;
predictions under-disperse, so the predicted band is compressed relative to the
...
OUTPNG      = r"C:\Users\zerru001\OneDrive - Wageningen University & Research\Paper1\RUN2\Graphs\000_RICEMETHANE\obspred_kor_gs_new2.png"  # copy to figs/obspred_kor_gs_new.png
...
pad  = 0.05 * (np.nanmax(allv) - np.nanmin(allv))
lim  = [np.nanmin(allv) - pad, np.nanmax(allv) + pad]   # full data range on both axes, 5% margin
