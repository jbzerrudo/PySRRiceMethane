...
REM --- give this run half the logical processors ---
set /a HALF=%NUMBER_OF_PROCESSORS%/2
...
echo ============================================================
echo  Python : %PYEXE%
echo  Cores  : %HALF% of %NUMBER_OF_PROCESSORS%
echo  Config : %CFG%
echo  Log    : %LOG%
echo ============================================================
echo Running. KEEP THIS WINDOW OPEN until it says Finished.
echo.
"%PYEXE%" GAM_RF_union_parallel.py "%CFG%" > "%LOG%" 2>&1
set "RC=%ERRORLEVEL%"
echo.
if not "%RC%"=="0" (
