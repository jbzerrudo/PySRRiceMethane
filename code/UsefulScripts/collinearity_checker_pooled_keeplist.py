...
INPUT_CSV = r"C:\Users\zerru001\OneDrive - Wageningen University & Research\Paper1\RUN2\CSV\POOLED\Data\Pooled_retvars_gam1.csv"
OUTPUT_DIR = r"C:\Users\zerru001\OneDrive - Wageningen University & Research\Paper1\RUN2\COLLINCHECK\POOLED\Metadata"
SITE_NAME = "POOLED"  # used in output filenames; e.g. "SK-CRK-2016"

TARGET_COL = "F_CH4_F"
VIF_THRESHOLD = 10.0 # 5.0 or 10.0
...
IMPORTANCE_CSV = r"C:\Users\zerru001\OneDrive - Wageningen University & Research\Paper1\RUN2\GAMRF\POOLED\Metadata\predictor_perm_importance_20260728_165317.csv"
...
# Physical primaries NEVER dropped by VIF, even above threshold. Motivated by the
# three site equations: temperature = es (KOR), water = AUC (PHL/JPN), demand = VPD (JPN).
# Swap es->Tair or add AUC_wet if you prefer; set [] to disable protection.
PROTECTED = ["es", "AUC", "VPD"]
...
def analyze_collinearity(df, target_col, threshold=10.0, outdir=None,  # change threshold to 5.0 for stricter elimination
    ...
    while True:
        ...
        if not np.isfinite(max_vif):
            ...
            inf_drop = [f for f in inf_vars if f not in PROTECTED] or inf_vars
            if importance_rank and len(inf_drop) > 1:
                # Drop the least important among the (non-protected) inf-VIF variables
                drop_feature = min(inf_drop, key=lambda f: importance_rank.get(f, float("inf")))
            else:
                drop_feature = inf_drop[0]
        ...
        if max_vif > threshold:
            ...
            above = [f for f in above if f not in PROTECTED]   # never drop protected primaries

            if not above:
                # only PROTECTED vars remain above threshold -> accept them and stop
                iteration_logs.append({
                    "iteration": iteration, "vif_table": vif_data.copy(),
                    "dropped": None, "dropped_vif": None,
                    "top_correlates": None, "drop_reason": None,
                })
                print(f"  Iter {iteration}: remaining VIF>{threshold} are all protected. Done.")
                break
            ...
            if importance_rank and len(above) > 1:
                ...
                # drop highest VIF among the non-protected above-threshold set
                drop_feature = max(above, key=lambda f: float(vif_data.loc[vif_data["feature"] == f, "VIF"].iloc[0]))
                drop_vif = float(vif_data.loc[vif_data["feature"] == drop_feature, "VIF"].iloc[0])
