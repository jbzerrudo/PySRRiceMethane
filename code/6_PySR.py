...
# ─── Configuration ────────────────────────────────────────────────────────────
INPUT_DIR = r"C:\Users\zerru001\OneDrive - Wageningen University & Research\Paper1\RUN2\CSV\POOLED\Data-Metadata"
OUTPUT_DIR = r"C:\Users\zerru001\OneDrive - Wageningen University & Research\Paper1\RUN2\PYSR\POOLED\Metadata\rerun_10Aug2026"
INPUT_FILE = "POOLED_retvars_pass2_ISO_C.csv"
TARGET_COL = "F_CH4_F"
EXCLUDE_HEADERS = {"site", "w", "Date", "Deltime", "time", "F_CH4_F_orig"}
MISSING_FLAGS = [-9999, -999900, -99999]
NITERATIONS = 2000       # or: 10000
MAXSIZE = 40             # v6.4: was 35. ORYZA embedding wants 10-20 nodes, so the top third of the
                         # old front was unusable, and near-duplicate complexity-30+ variants
                         # fragment the cross-seed recurrence count without adding information.
POPULATION_SIZE = 50
N_POPULATIONS = 40       # v6.4: was hard-coded 20 at the PySRRegressor call. The mountains paper
                         # runs 40 x 60 = 2400 individuals against this script's former 20 x 50 =
                         # 1000. More populations is the cheapest way to raise the chance that any
                         # single seed independently finds the true form, which is what the
                         # cross-seed recurrence count measures.
BINARY_OPERATORS = ["+", "-", "*", "/"]
...
# v6.4: removed `tanh`. Measured on the Mase run of 2 Aug 2026: the recurring term
# tanh(0.027*AUC_wet - Tv_K) was 95.85% saturated (argument range -297 to +7.9), so the
# search was buying a step function, not a curve; Tv moved the output by more than 0.01
# in only 12.34% of rows. It appeared in every equation from complexity 14 upward and
# displaced the exponential entirely — zero occurrences of exp in that seed's whole
# front. The mountains paper carries no tanh and recovered the exponential in 8 of 12
# runs. Re-add only with a declared reason.
...
NESTED_CONSTRAINTS = {
    # v6.4: exp-in-exp was 1, now 0. The Celsius run of 3 Aug 2026 used that
    # allowance in BOTH gate-clearing candidates: #25 (CV +0.629) and #15
    # (CV +0.571) each contain exp(exp(Tv_C*(AUC_wet*(Tv_C*1.619e-7)))).
    # exp(exp(x)) has no physical basis for a CH4 flux and is indefensible in
    # review, whatever its CV.
    "exp":  {"exp": 0, "log": 0},        # no exp inside exp; no log inside exp
    "log":  {"log": 0, "exp": 1},        # allow one nested exp inside log (e.g., log(1+exp(x)))
    # v6.4: the "tanh": {"tanh": 0} entry was removed together with tanh itself.
    # PySR serialises these keys into a Julia Dict for the backend, so a key naming
    # an operator that is not in the search is at best dead config. If you ever put
    # tanh back into UNARY_OPERATORS, put this line back with it.
}
...
CONSTRAINTS = {
    ...
    "log":  8,
    # v6.4: "tanh": 8 removed with the operator. Restore both together.
    "sqrt": 10,
    "+":    (-1, -1), # unconstrained
    "-":    (-1, -1),
    "*":    (-1, -1),
...
TEST_FRACTION = 0.2
# v6.4: ONE day split shared by every seed. Previously the split used `seed`, so
# the seed drove both the PySR search AND which days landed in the hold-out, and
# the two could not be told apart. Measured on Mase: the complexity-1 loss, which
# is just the training-set variance of F_CH4_F, ran from 11.11 (seed 50) to 30.57
# (seed 46), a factor of 2.75. Rule A's R2 correlated with that variance at
# r = +0.735, p = 0.0064 (Celsius run) and r = +0.752, p = 0.0047 (Kelvin run).
# Rule A was ranking the split as much as the equation. With SPLIT_SEED fixed,
# seed-to-seed variation isolates search stochasticity, which is the only thing
# a multi-seed design should be measuring.
SPLIT_SEED = 42
PARSIMONY = 0.0032
# v6.4: was True. PySR's default is False and it exists for datasets too large to
# evaluate whole; Mase is 7,041 rows. It does not affect reported losses (the hall
# of fame is always scored on full data) but it does add noise to the evolutionary
# path, so seeds diverge in WHICH forms they find. Cross-seed agreement is now the
# selection criterion, so manufacturing disagreement is the wrong trade.
BATCHING = True
BATCH_SIZE = 1024
# v6.4: predictors to exclude from the search even though they survived the union
# and the VIF. WD is wind direction in degrees, 0 to 360, so a linear term treats
# 359 deg and 1 deg as opposite extremes although they are the same wind. The
# Celsius run put it in both gate-clearing equations, once as Tv_C - 0.7609*WD and
# once in a denominator against a bounded tanh. The wraparound-free form of the
# same information is already in the design as uzonal and vmerid, and vmerid was
# itself rejected by the union at Mase, which is evidence direction is weak there.
# This is box 7b declared pruning ("raw circular WD excluded") applied up front.
DROP_PREDICTORS = ["WD"]
# v6.4: selection rule. A form must appear on the Pareto front of at least this
# many of the N_SEEDS independent runs before stage 8 will validate it. Declare
# this BEFORE looking at a run's output. 6 of 12 is a simple majority.
# Replaces Rule A, which crowned the seed whose single best candidate scored
# highest on the 80/20 hold-out: a maximum over 12 seeds x 4 candidates = 48
# evaluations, so an order statistic rather than a performance estimate, and it
# made one non-bit-reproducible run load-bearing. Rule A is still computed and
# written to seed_selection_summary_*.txt for comparison; it no longer selects.
MIN_RECURRENCE_RUNS = 6
# v6.4.1: if the merged recurrence front has this many usable forms or fewer,
# stage 8 validates ALL of them instead of sub-selecting four. Stage 8 is a
# curve fit and costs minutes, so there is nothing to save by dropping forms.
MAX_RECUR_SLOTS = 8
N_DISPLAY = 15
DPI = 300
...
def run_symbolic_regression(csv_path, out_root, run_ts, seed=None, outdir=None):
    ...
    # v6.4: declared pruning, see DROP_PREDICTORS in the config block.
    pruned = [c for c in DROP_PREDICTORS if c in predictors]
    if pruned:
        predictors = [c for c in predictors if c not in DROP_PREDICTORS]
        print(f"  Declared pruning: dropped {pruned}")
    missing = [c for c in DROP_PREDICTORS if c not in pruned]
    if missing:
        print(f"  DROP_PREDICTORS lists {missing}, not present in this file.")
    ...
    if dates_full is not None:
        ...
        rng = np.random.RandomState(SPLIT_SEED)   # v6.4: was seed; see SPLIT_SEED
        ...
        X_train, X_test, y_train, y_test = train_test_split(
            X, y, test_size=TEST_FRACTION, random_state=SPLIT_SEED)  # v6.4: was seed
    ...
    _base_kwargs = dict(
        ...
        populations=N_POPULATIONS,   # v6.4: was hard-coded 20
...
def _structural_signature(eq_str, predictors):
    """Canonical form of an equation with every fitted constant replaced by a
    single placeholder C.

    This is what makes a cross-seed count possible: two seeds that find the
    same structure will fit different constants to it, so the raw strings never
    match, but the signatures do. c1*AUC_wet*exp(c2*Tv_C) from seed 44 and
    seed 47 both collapse to C*AUC_wet*exp(C*Tv_C).

    Validated 3 Aug 2026 against the 12 Pareto fronts of the tropical-cyclone
    paper: it reproduces every recurrence count that paper reports, without
    being given them (exponential at complexity 8 in 8 of 12 runs; V0*exp(C*V0*t)
    in 2 at loss 95.754; (V0+t)*exp(C*t) in 6 at loss 113.158; the complexity-9
    leading-factor form in 7 at loss 97.089).

    Returns (signature, readable form), or (None, None) if it will not parse.
    """
    import sympy as sp
    expr = _safe_sympify(eq_str, predictors)
    if expr is None:
        return None, None
    C = sp.Symbol("C")
    expr = expr.xreplace({n: C for n in expr.atoms(sp.Float)})
    expr = expr.xreplace({r: C for r in expr.atoms(sp.Rational)
                          if r not in (sp.S.Zero, sp.S.One, sp.S.NegativeOne)})
    try:
        expr = sp.expand(expr)
    except Exception:
        pass
    return sp.srepr(expr), str(expr)


def _select_by_recurrence(seed_results, min_runs):
    """Selection rule: keep the FORMS that recur across independent seeds.

    Replaces Rule A, which picked the seed whose single best candidate scored
    highest on the 80/20 hold-out. That was a maximum over 12 seeds x 4
    candidates = 48 evaluations, so the winning number was an order statistic
    rather than a performance estimate, and it made one non-reproducible run
    load-bearing. This is the rule used in the tropical-cyclone paper, where
    twelve seeds are a robustness count over forms rather than a competition.

    Cross-seed loss comparison is only legitimate because SPLIT_SEED fixes one
    day split for every seed. Before that change each seed trained on different
    days and their losses were not on the same scale (measured range of the
    complexity-1 loss, which is just the training-set variance: 11.11 to 30.57).

    Returns (rep_eqs, counts_by_index, report_lines) or (None, None, lines) when
    too few forms recur, in which case the caller falls back to Rule A.
    """
    rows = []
    for r in seed_results:
        ...
        if eqs is None or len(eqs) == 0:
            continue
        for idx, x in eqs.iterrows():
            sig, form = _structural_signature(str(x["equation"]), r["predictors"])
            if sig is None:
                continue
            rows.append({"seed": r["seed"], "sig": sig, "form": form,
                         "complexity": float(x["complexity"]),
                         "loss": float(x["loss"]),
                         "equation": str(x["equation"])})
    lines = []
    if not rows:
        lines.append("  [WARN] no equation could be sympified; cannot count recurrence.")
        return None, None, lines

    pool = pd.DataFrame(rows)
    counts = pool.groupby("sig")["seed"].nunique()
    n_seeds = pool["seed"].nunique()

    lines.append(f"  {n_seeds} seeds, {len(pool)} equations, "
                 f"{pool['sig'].nunique()} distinct forms")
    lines.append(f"  promotion threshold: present in >= {min_runs} of {n_seeds} seeds")
    lines.append("")
    lines.append(f"  {'cplx':>4s} {'seeds':>7s} {'best loss':>11s}   form")
    lines.append(f"  {'-'*4} {'-'*7} {'-'*11}   {'-'*52}")

    keep = counts[counts >= min_runs].index
    # Representative of each kept form = its lowest-loss instance. Valid only
    # because every seed shares one split; see SPLIT_SEED.
    reps = (pool[pool["sig"].isin(keep)]
            .sort_values("loss")
            .groupby("sig", as_index=False)
            .first())
    if len(reps) == 0:
        lines.append("  (nothing reaches the threshold)")
        return None, None, lines

    reps["n_runs"] = reps["sig"].map(counts)
    reps = (reps.sort_values(["complexity", "loss"])
                .drop_duplicates(subset=["complexity"], keep="first")
                .reset_index(drop=True))

    # A recurrence-filtered set is NOT a Pareto front. Each entry is the best
    # instance of a form that happened to recur, so loss can RISE with
    # complexity: on the Mase run of 3 Aug, exp(C*AUC_wet*Tv_C) recurred in 10
    # of 12 seeds at complexity 6 with loss 8.31 while C*AUC_wet*Tv_C recurred
    # in 12 of 12 at complexity 5 with loss 4.00. select_candidate_equations
    # and its knee geometry both assume a monotone front, so filter first.
    dominated = []
    keep, best = [], float("inf")
    for i, r in reps.iterrows():
        if r["loss"] < best:
            ...
            dominated.append(i)
    for i, r in reps.iterrows():
        tag = "" if i in keep else "   dominated"
        lines.append(f"  {int(r.complexity):4d} {int(r.n_runs):3d}/{n_seeds:<3d} "
                     f"{r.loss:11.4f}   {r.form}{tag}")
    if dominated:
        lines.append("")
        lines.append(f"  {len(dominated)} recurring form(s) dominated: a simpler "
                     f"recurring form already fits better.")
    reps = reps.loc[keep].reset_index(drop=True)

    if len(reps) < 3:
        lines.append("")
        lines.append(f"  [WARN] only {len(reps)} non-dominated recurring form(s). "
                     f"select_candidate_equations needs at least 3 to locate a knee.")
        return None, None, lines

    # PySR's own `score` is the marginal log-loss gain relative to the PREVIOUS
    # point of that seed's front, so it is meaningless on a merged front.
    # Recompute it here on the merged front, by PySR's definition.
    loss = reps["loss"].to_numpy(float)
    cplx = reps["complexity"].to_numpy(float)
    score = np.zeros(len(reps))
    with np.errstate(divide="ignore", invalid="ignore"):
        score[1:] = -(np.diff(np.log(loss)) / np.diff(cplx))
    reps["score"] = np.nan_to_num(score, nan=0.0, posinf=0.0, neginf=0.0)

    lines.append("")
    lines.append(f"  merged front: {len(reps)} recurring forms, "
                 f"complexity {int(cplx.min())} to {int(cplx.max())}")
    return reps, dict(zip(reps.index, reps["n_runs"])), lines
...
def run_multiseed_pipeline(csv_path, out_root, run_ts, n_seeds, base_seed=42):
    ...
    # ── Selection: cross-seed form recurrence (v6.4) ─────────────────────
    # Rule A is still computed and written above, so the two can be compared,
    # but it no longer decides what stage 8 validates.
    predictors = rule_a_winner["result"]["predictors"]

    print(f"\n{'#' * 80}")
    print(f"# FORM RECURRENCE ACROSS SEEDS  (selection rule, v6.4)")
    print(f"{'#' * 80}")
    reps, rep_counts, rec_lines = _select_by_recurrence(
        seed_results, MIN_RECURRENCE_RUNS)
    for ln in rec_lines:
        print(ln)

    rec_path = sr_root / f"form_recurrence_{run_ts}.txt"
    with open(rec_path, "w", encoding="utf-8") as f:
        f.write("Cross-seed form recurrence\n")
        f.write(f"Run timestamp: {run_ts}\n")
        f.write(f"Threshold: MIN_RECURRENCE_RUNS = {MIN_RECURRENCE_RUNS}\n\n")
        f.write("\n".join(rec_lines) + "\n")
        f.write("\nEvery fitted constant is replaced by one placeholder before\n"
                "counting, so seeds that find the same structure with different\n"
                "constants are counted together. A single placeholder cannot tell\n"
                "WHERE a constant sits, so one algebraic family can appear on two\n"
                "lines; counts are a lower bound.\n")
    print(f"\n  [SAVED] {rec_path.name}")

    forms = {}
    if reps is not None:
        # v6.4.1: with a short merged front there is no reason to sub-select.
        # select_candidate_equations exists so stage 8 need not CV a 28-equation
        # PySR front. On the 6-entry recurrence front of the Mase run of 4 Aug
        # its four selectors landed on complexity 3, 5 and 7 with the fourth
        # colliding, so complexity 6 was dropped -- the form every one of the 12
        # seeds found, and the lowest loss of any 12/12 form. Stage 8 is a curve
        # fit, so validate them all and let the CV decide.
        usable = reps[reps["complexity"] > 1]      # complexity 1 is a constant
        if len(usable) <= MAX_RECUR_SLOTS:
            candidates = [(i, f"c{int(reps.loc[i, 'complexity']):02d}")
                          for i in usable.index]
            print(f"\n  Merged front has {len(usable)} usable form(s); "
                  f"validating all of them.")
        else:
            candidates = select_candidate_equations(reps)
            print(f"\n  Merged front has {len(usable)} usable form(s), above "
                  f"MAX_RECUR_SLOTS={MAX_RECUR_SLOTS}; sub-selecting four.")
        forms = _build_forms_from_run(
            candidates, reps, predictors, seed=-1, origin="recur")
        # Replace the per-form provenance with the real recurrence counts.
        for cidx, clabel in candidates:
            if clabel in forms:
                forms[clabel][0]["consensus_count"] = int(reps.loc[cidx, "n_runs"])
                forms[clabel][0]["n_seeds"] = int(reps.loc[cidx, "n_runs"])
                forms[clabel][0]["source_seed"] = "recurring"
        print(f"  Recurring forms going to stage 8:")
        for clabel, fl in forms.items():
            print(f"    {clabel:<32} complexity {fl[0]['complexity']:.0f}  "
                  f"in {fl[0]['n_seeds']} of {len(seed_results)} seeds")
    else:
        print(f"\n  [FALLBACK] Too few forms recur at MIN_RECURRENCE_RUNS="
              f"{MIN_RECURRENCE_RUNS}.")
        print(f"             Only Rule A's candidates will be validated. "
              f"Say so in the methods if you report this run.")

    # v6.4.1: Rule A's four candidates are ALWAYS validated too, tagged
    # origin="ruleA". _write_auto_stage8 namespaces every slot as
    # "<prefix>_<origin>", so the two selection rules appear side by side in
    # cv_summary.txt and are scored on the SAME day-grouped folds. That is the
    # comparison the methods section needs; neither rule is being trusted on
    # its own hold-out number.
    ruleA_forms = _build_forms_from_run(
        rule_a_winner["result"]["candidates"],
        rule_a_winner["result"]["eqs"],
        predictors, rule_a_winner["seed"], origin="ruleA")
    for clabel, fl in ruleA_forms.items():
        forms.setdefault(clabel, []).extend(fl)
    print(f"\n  Rule A forms going to stage 8 (seed {rule_a_winner['seed']}):")
    for clabel, fl in ruleA_forms.items():
        print(f"    {clabel:<32} complexity {fl[0]['complexity']:.0f}")
    print(f"\n  Total slots for stage 8: "
          f"{sum(len(v) for v in forms.values())}")
