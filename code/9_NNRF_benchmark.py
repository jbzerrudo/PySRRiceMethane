...
EQUATIONS, all verbatim from the arm's own report
  ...
  PHL  complex_ruleA        cplx 24  CV +0.598  run_20260806_104044 seed 53
...
def eq_phl(d):
    """IRRI, complex_ruleA, complexity 24, CV +0.598."""
    Tv = d["Tv_C"].to_numpy(float)
    uz = d["uzonal"].to_numpy(float)
    SRH = d["SR*HODsin"].to_numpy(float)
    hwet = d["hwet"].to_numpy(float)
    SRTs = d["SR*Ts"].to_numpy(float)
    AUCd = d["AUC_dry"].to_numpy(float)
    a = Tv + ((((4.2946577 - uz) - (SRH * 0.017328158)) / np.exp(hwet)) - 24.07842)
    return a * np.exp((np.sqrt(SRTs) - AUCd) * 0.004744538) + 0.8820041
...
ARMS = {
    ...
    "PHL": dict(
        ...
        form="complex_ruleA cplx 24", pysr_cv_r2=0.598),
    "POOLED": dict(
